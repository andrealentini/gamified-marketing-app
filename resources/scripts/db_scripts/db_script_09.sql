...
/* Creation of master_questionnaire table. */
DROP TABLE IF EXISTS `master_questionnaire`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `master_questionnaire` (
	`id` INT NOT NULL AUTO_INCREMENT,
    `marketing_section` INT NOT NULL,
    `statistical_section` INT NOT NULL,
...
/* Data inserts for master_questionnaire table. */
...
CREATE TABLE `filling` (
    ...
    `master_questionnaire` INT NOT NULL,
...
/* Triggers */
DROP TRIGGER IF EXISTS `master_questionnaire_insert_consistency`;
DELIMITER $$
CREATE TRIGGER `master_questionnaire_insert_consistency`
BEFORE INSERT ON `master_questionnaire`
FOR EACH ROW
BEGIN
	DECLARE m_type INT;
    DECLARE s_type INT;
    SELECT M.`type`
	INTO m_type
	FROM `questionnaire` M
	WHERE M.`id` = new.`marketing_section`;
    SELECT S.`type`
	INTO s_type
	FROM `questionnaire` S
	WHERE S.`id` = new.`statistical_section`;
    IF m_type = 1 THEN SIGNAL SQLSTATE '45001'; END IF;
    IF s_type = 0 THEN SIGNAL SQLSTATE '45001'; END IF;
END
$$

DROP TRIGGER IF EXISTS `master_questionnaire_update_consistency`;
DELIMITER $$
CREATE TRIGGER `master_questionnaire_update_consistency`
BEFORE UPDATE ON `master_questionnaire`
FOR EACH ROW
BEGIN
	DECLARE m_type INT;
    DECLARE s_type INT;
...
DROP TRIGGER IF EXISTS `offensive_words_check_insert`;
DELIMITER $$
CREATE TRIGGER `offensive_words_check_insert`
BEFORE INSERT ON `answer`
FOR EACH ROW
BEGIN
	DECLARE i INT;
    DECLARE myWord VARCHAR(45);
    SELECT MIN(`id`) INTO i FROM `offensive_word`;
	myloop: LOOP
                IF (SELECT MAX(`id`) FROM `offensive_word`) = i THEN LEAVE myLoop; END IF;
				SELECT `word` INTO myWord FROM `offensive_word` WHERE `id` = i;
                IF myWord != NULL THEN
					IF (SELECT 1 WHERE new.`text` REGEXP CONCAT('\s*', myWord, '\s*')) = 1 THEN SIGNAL SQLSTATE '45001'; END IF;
				END IF;
                SET i = i + 1;
			END LOOP myloop;
END
...
/* Default inserts */
INSERT INTO `role` VALUES (1, "ADMIN", "Administrator of the platform");
INSERT INTO `registered_user` VALUE (1, "admin", "admin", "admin@gamifiedmarketingapp.com", null, null, false, 1);
INSERT INTO `questionnaire` VALUE (1, false);
